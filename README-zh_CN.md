...
对于*忽略顶层的独立标题*设置，如果顶层只有一个标题，则在构建有序列表时将其排除。此设置包含*基于现有的最高级别*，但它处理得更为“激进”。例如：

| 默认 | 忽略顶层的独立标题 |
| :--: | :------------: |
| ![默认](images/default.jpg) | ![忽略顶层的独立标题](images/ignore-single-heading.jpg) |

##### 忽略的最大数量

对于已启用：忽略顶层的独立标题。顶层可忽略标题的最大数量。例如：

| 默认 | 忽略顶层的独立标题，使用默认值(`6`) | 忽略的最大数量为 `1` |
| :--: | :--------------------------: | :---------------: |
| ![默认](images/no-ignore.jpg) | ![忽略顶层的独立标题](images/ignore-default.jpg) | ![忽略的最大数量](images/ignore-with-number-1.jpg) |
