...
對於*忽略頂層的獨立標題*設定，如果頂層只有一個標題，則在構建有序列表時將其排除。此設定包含*基於現有的最高級別*，但它處理得更為“激進”。例如：

| 預設 | 忽略頂層的獨立標題 |
| :--: | :------------: |
| ![預設](images/default.jpg) | ![忽略頂層的獨立標題](images/ignore-single-heading.jpg) |

##### 忽略的最大數量

對於已啟用：忽略頂層的獨立標題。頂層可忽略標題的最大數量。例如：

| 預設 | 忽略頂層的獨立標題，使用預設值(`6`) | 忽略的最大數量為 `1` |
| :--: | :--------------------------: | :---------------: |
| ![預設](images/no-ignore.jpg) | ![忽略頂層的獨立標題](images/ignore-default.jpg) | ![忽略的最大數量](images/ignore-with-number-1.jpg) |
