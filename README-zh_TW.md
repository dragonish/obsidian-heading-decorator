...
- **裝飾器的模式**：切換此設定可將標題裝飾為有序列表、獨立層級、分層拼接或無序列表。
  - **有序列表**：按照裝飾器樣式將當前標題級別與頂層標題級別使用分隔符連線起來以構建結果，類似於[預覽](#預覽)中展示的效果。
  - **獨立層級**：預設情況下，按照各自級別裝飾器樣式將當前標題級別構建結果。
  - **分層拼接**：按照各自級別裝飾器樣式，將當前標題級別與頂層標題級別使用分隔符連線起來以構建結果。
  - **無序列表**：直接將當前標題級別構建為固定字串。
- **裝飾器的不透明度**：設定標題裝飾器的不透明度。值是百分比的形式。
- **裝飾器的位置**：設定標題裝飾器的位置。您可以將內容配置為顯示在標題之前或之後。
- **所識別的最大級別**：指定所識別標題的最大級別，不處理高於此級別的標題。

不同裝飾器模式示例：

| 有序列表 (十進位制數字) | 獨立層級 | 分層拼接 | 無序列表 (使用 `H1 H2 H3 H4 H5 H6`) |
| :---------------: | :------: | :-----: | :-------------------------------: |
| ![有序列表](images/ordered.jpg) | ![獨立層級](images/independent.jpg) | ![分層拼接](images/splice.jpg) | ![無序列表](images/unordered.jpg) |
...
將裝飾器的模式設定為獨立層級時出現的設定，指示在標題級別超出指定級別後按有序列表進行構建。例如：

| 預設 (*設定值為 `6`*) | 設定值為 `4` |
| :----------------: | :---------: |
| ![預設](images/independent.jpg) | ![設定值為 4](images/independent-4.jpg) |

### 裝飾器樣式

您可以控制計數器的樣式型別和分隔符。存在兩種特殊的計數器樣式：

- **自定義列表樣式**：為裝飾器設定自定義列表樣式。使用空格分隔條目。
- **指定字串**：為裝飾器設定指定的字串。
...
在將裝飾器的模式設定為有序列表、獨立層級或分層拼接時，您可以控制標題的渲染邏輯，用於調整渲染結果或者忽略掉不想處理的標題級別。
...
對於*基於現有的最高級別*設定，將使用筆記中最高級別的標題作為構建裝飾器的基礎。例如：
...
在構建裝飾器時將頂層的標題排除掉，受*忽略的最大層級數量*選項控制。例如：
...
對於*忽略頂層的獨立標題*設定，如果頂層只有一個標題，則在構建裝飾器時將其排除，受*忽略的最大層級數量*選項控制。此設定包含*基於現有的最高級別*，但它處理得更為“激進”。例如：
...
每個裝飾元素都有一個與當前標題級別相關聯的屬性 `[data-decorator-level="n"]` (*`n` 為標題級別*)。
